...
def switch_event(event):
    global counter, show_correct_answer
    ...
    if event == RotaryEncoder.CLOCKWISE:
        ...
        print("Button down")
        show_correct_answer = True  # Set the flag to True when the button is pressed
...
def pak_gebruiker_keuze() -> int:
    while True:
        ...
        if gebruiker_keuze in range(1, 5):
            ...
            print("Invalid antwoord, Voer nummer van je keuze in.")

def speel_spel(amount: int, catogory: int, encoder_instance: RotaryEncoder) -> None:
    global punten, show_correct_answer
    global counter
    ...
    for vraag in ophalen:
        ...
        if show_correct_answer:  # Check if the flag is True
            long_string(display, juiste_antwoord_tekst, 2)
            show_correct_answer = False  # Reset the flag after displaying the correct answer
